...
const BosMaker = artifacts.require('BosMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('BosMaker', ([alice, bar, account, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.bos = await BosToken.new(/*account, 1917820800, */{ from: alice });
        await this.bos.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await BosMaker.new(this.factory.address, bar, this.bos.address, this.weth.address);
        this.bosWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.bos.address)).logs[0].args.pair);
    ...
    it('should make BOSs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.bosWETH.address, '10000000', { from: minter });
        await this.bos.transfer(this.bosWETH.address, '10000000', { from: minter });
        await this.bosWETH.mint(minter);
        ...
        // After calling convert, bar should have BOS value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.bos.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for BOS-ETH pair
        await this.bos.transfer(this.bosWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.bosWETH.address, '100000', { from: minter });
        await this.bosWETH.sync();
        await this.bos.transfer(this.bosWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.bosWETH.address, '10000000', { from: minter });
        await this.bosWETH.mint(minter);
        assert.equal((await this.bosWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.bos.address, this.weth.address);
        assert.equal((await this.bos.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.bosWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});